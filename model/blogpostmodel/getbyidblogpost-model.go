...
func (m *modelSqlx) GetByID(ctx context.Context, id int64) (*entities.BlogPostResponse, *errors.CustomError) {
	...
	err := m.sqlx.GetContext(ctx, &blogPost, `SELECT BlogPosts.id, BlogPosts.title, BlogPosts.content, BlogPosts.published, BlogPosts.updated, user.id AS "user.id", user.displayName AS "user.displayName", user.email AS "user.email", user.image AS "user.image" FROM BlogPosts INNER JOIN Users AS user ON user.id = userId INNER JOIN PostCategories ON PostCategories.postId = BlogPosts.id INNER JOIN Categories as categories ON categories.id = PostCategories.categoryId`)
	if err != nil {
